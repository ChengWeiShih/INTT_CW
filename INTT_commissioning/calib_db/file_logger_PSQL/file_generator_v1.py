...
def print_map():
    ...
    if (ROC1_entry.get() !=""):
        print('# ROC1',ROC1_entry.get())
    if (ROC2_entry.get() !=""):
        print('# ROC2',ROC2_entry.get())
    if (note1_entry.get() != ""):
        print('#',note1_entry.get())
    if (note2_entry.get() != ""):
        print('#',note2_entry.get())
...
button_confirm_map = tk.Button(root, text="Confirm", fg='red', command=apply_map)
button_confirm_map.grid(row=n_row, column=9 )
...
ROC1_label.grid(row=n_row, column=n_column,padx=15)

ROC1_entry = tk.Entry(root,width=10,textvariable=tk.StringVar(value=""))
ROC1_entry.grid(row=n_row+1, column=n_column, rowspan = 2, padx=15)
...
ROC2_label.grid(row=n_row, column=n_column,padx=15)
ROC2_entry = tk.Entry(root,width=10,textvariable=tk.StringVar(value=""))
ROC2_entry.grid(row=n_row+1, column=n_column, rowspan = 2, padx=15)
