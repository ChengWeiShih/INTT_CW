...
{
    public :
        Multiplicity(TString ladder_name, int ROC_index, TString ladder_port, int Felix_ch, bool set_logY, int chip_N_bin, int chip_L_edge, int chip_R_edge, int HL_factor);
...
Multiplicity::Multiplicity(TString ladder_name, int ROC_index, TString ladder_port, int Felix_ch, bool set_logY, int chip_N_bin, int chip_L_edge, int chip_R_edge, int HL_factor)
...
HL_factor(HL_factor)
{
    ...
    {
        event_hit_hist[hid] = (hid == 26) ? new TH1F("","", chip_N_bin * HL_factor, chip_L_edge, chip_L_edge + (chip_N_bin * HL_factor) * ((chip_R_edge - chip_L_edge)/chip_N_bin)) : new TH1F("", "", chip_N_bin, chip_L_edge, chip_R_edge);
        ...
        event_hit_hist[hid] -> SetTitle( (hid == 26) ? ("Chip level, all chips") : Form("Chip level, U%i",hid+1));
...
void Multiplicity::Draw_plot(TString plot_folder_dire, TString set_name)
{
    ...
    TString output_plot_name = Form("multiplicity_%i_%s",Felix_ch,ladder_name.Data());
    output_plot_name += (set_logY == true) ? "_log" : "_linear";    

    c1 -> Print( Form("%s/%s.pdf(", plot_folder_dire.Data(), output_plot_name.Data()) );
	for (int i = 0; i < 27; i++) // note : chip + total
    {	
		event_hit_hist[i] -> Draw("hist");
...
{
    TFile * file_output = new TFile(Form("%s/%i_%s_multiplicity_hist.root", plot_folder_dire.Data(), Felix_ch, ladder_name.Data()), "RECREATE");
    file_output -> cd();
