...
vector<clu_info> clustering(vector<hit_info> single_event, int bco, long long bco_full, int FC_id)
{
    ...
    for (int i = 0; i < single_event.size(); i++) // note : number of hits in this event, for this half-ladder
    {
        ...
        {
            cout<<"chip_id : "<<chip_conv<<" chan_id : "<<chan_conv<<" fired more than once ! "<<bco<<" "<<bco_full<<" "<<FC_id<<endl;
...
void multiplicity_ana ( TString mother_folder_directory, TString set_name, string full_file_directory, TString server_name, int FC_id, bool set_logY, int chip_N_bin, int chip_L_edge, int chip_R_edge, int HL_factor)
{
    ...
    cout<<set_name<<", "<<server_name<<"_"<<FC_id<<endl;
    ...
    for (long long i = 0; i < N_hit; i++)
    {
        ...
        if (chip_id > 0 && chip_id < 27 && chan_id > -1 && chan_id < 128 && module == FC_id) // todo : single ladder mode
        {
            ...
            {
                clu_vec = clustering(single_event_hit_vec, BCO_buffer, BCO_full_buffer, FC_id);
