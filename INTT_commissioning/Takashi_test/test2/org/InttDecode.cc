...
void fillBcoHitBuf(deque<int>& vHitBuf, deque<BcoHitBuf*>* vBcoHitBuf, bool debug=false)
{
    ...
    while(vHitBuf.size()>0){
        ...
        if((w&0xFF00FFFF) == 0xAD00CADE) {   // find hit header
            ...
            // reconstruct new BCO
            Long64_t ltmp = ((w>>16)&0xFF);
            Long64_t BCO  = (ltmp<<32);
...
bool checkReadyToDecode(map<Long64_t, EventBcoHitBuf*>& vEvtBcoHitBuf){
...
void  InttPacket::dump ( ostream& os )
{
    for ( int iladder = 0; iladder < 14; iladder++){
        cout<<iladder<<" "<< eventNum[iladder]<<endl;
    }
...
Long64_t InttPacket::lValue(const int hit, const int field)
{
    if ( hit < 0 || hit >= (int) intt_hits.size()) return 0;
...
long long  InttPacket::lValue(const int hit, const char *what)
{
...
/////////////
InttDecode::InttDecode(int pID=3001, fileEventiterator* eventItr)
...
InttPacket* InttDecode::getNextPacket()
{
  ...
  while(1){// infinite loop
      ...
      if(readyToDecode){
          ...
          packet = new InttPacket(firstEvtBuf);
          //packet->dump();
      ...
      oncsSubevent *p = (oncsSubevent*)e->getPacket(3001);
      if(p){
        ...
        cout<<i<<"  Evt:"<<e->getEvtSequence()<<" "<<length<<" "<<nwout<<" "<<state<<endl;
            ...
            for(int i=0; i<bufsize; i++){
               ...
               //     cout<<"size : "<<vBcoHitBuf[ilad].size()<<endl;
                ...
                if(bcoHitBuf->done){
                    ...
                    map<Long64_t, EventBcoHitBuf*>::iterator itr = vEvtBcoHitBuf.find(bcoHitBuf->bco);
