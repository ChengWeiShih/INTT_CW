...
selected_run(selected_run)
{
    ...
    vector< TF1 * > fit_unit(6, NULL); // todo : the number of fitting for each unit is here, now 6
    ...
    if (ana_level == "chip") // note : 26 chips + 1 total in the half-ladder
    {
        ...
        for (int i = 0; i < check_number; i++) // note : 26 chips + 1 total in the half-ladder
        {
            ...
            {
                DAC_hist_bin[i][i1] = new TH1F("","",8,0,8); // todo : NOT TO change here if you want to only show 7 bins
...
void LadderDAC::Fit()
{
    ...
    for (int i = 0; i < DAC_hist_all.size(); i++) {
        ...
        DAC_hist_all[i] -> SetBinContent(1,0); //todo : first bin too noisy, set to zero
...
{    
    TCanvas * c_bin = new TCanvas("c_bin","c_bin",1100,800);
    ...
    TString bin8_txt = (bin8_set_zero == true) ? "_no_bin8" : "";

    c_bin -> Print( Form("%s/DAC_scan_matrix_%s%s.pdf(", plot_folder_dire.Data(), ladder_name.Data(), bin8_txt.Data()) );
	for (int i = 0; i < DAC_hist_bin.size(); i++)
    {	
        // cout<<"size test "<<DAC_hist_bin.size()<<endl;
        c_bin -> Divide(4,3); // todo : the number of the plots are fixed, 8 runs are maximum
        ...
        c_bin -> cd(11);
        draw_text -> DrawLatex(0.0, 0.5, Form("%s",set_name.Data()));
        if (i == DAC_hist_bin.size() - 1 ){
            draw_text -> DrawLatex(0.0, 0.35, Form("FC#%i, Port %s, All chips",Felix_ch,ladder_port.Data()));
        }
        else {
            draw_text -> DrawLatex(0.0, 0.35, Form("FC#%i, Port %s, U%i",Felix_ch,ladder_port.Data(),i+1));
        }
        if (bin8_set_zero == true){
            draw_text -> DrawLatex(0.0, 0.2, Form("Bin8 set content 0"));
        }
...
void LadderDAC::Output_comb_plots(TString plot_folder_dire, TString set_name)
{
    ...
    {	
        ...
        draw_text -> DrawLatex(0.16, 0.91, Form("%s, FC#%i, Port %s",set_name.Data(),Felix_ch,ladder_port.Data()));
