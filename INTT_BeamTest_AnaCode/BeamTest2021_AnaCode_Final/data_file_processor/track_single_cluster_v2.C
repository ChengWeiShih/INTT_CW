...
// note : v2 : apply the correction on all the slot, based on the fit slope
...
// note : chip 1 to chip 5
// note : the value is given by the fit, please take a look at the slide 'INTT_2022_10_29_Beamtest_noiselevel.key'
// note : the reason why I make it time the -1 is for correcting the sign.
// todo : change here the alignment slope
double correction_slope_B (int chip_pos)
{
	// return -1 * ( -0.037 * chip_pos + 0.540 ); // note : from run33, message by CW, 2022/11/09 (run89 should use this one to correct the type B region)
	return -1 * ( -0.0382944 * chip_pos + 0.626502 ); // note : for run64, 2023/02/10
}
// todo : change here the alignment slope
// note : chip 6 ~ chip 13
// note : the value is given by the fit, please take a look at the slide 'INTT_2022_10_29_Beamtest_noiselevel.key'
double correction_slope_A (int chip_pos)
{
	// return -1 * ( -0.030 * chip_pos + 0.534 ); // note : for run52
	// return -1 * ( -0.029 * chip_pos + 0.5885 ); // note : for run64, based on the estimation

	// return -1 * ( -0.0261907 * chip_pos + 0.547567 ); // note : for run89, 2023/02/10
	return -1 * ( -0.0323918 * chip_pos + 0.59931 ); // note : for run64, 2023/02/10
...
cluster_str data_all_edep_weight (vector<int> data_all[4][26],int selected_chip, double adc_convert[8] )
{
	...
	for (int l1=0; l1<4; l1++)
	{
		...
		for (int l4=0; l4<chan_master.size(); l4++) 
		{
			...
			// note : only applies the correction on layer-1
			// note : the amount of the correction is given by the fit slope
			if (l1 == 1) 
			{
				if (selected_chip > 0 && selected_chip < 6) // note : 1 ~ 5
				{
					chan_master[l4] = chan_master[l4] + correction_slope_B(selected_chip);
				}
				else if (selected_chip > 5 && selected_chip < 14) // note : 6 ~ 13
				{
					chan_master[l4] = chan_master[l4] + correction_slope_A(selected_chip);
				}
				else 
				{
					cout<<"correction wrong"<<endl;
				}
...
void track_single_cluster_v2 (TString file_name) // by analyzing the tree_both
{
	...
	// todo : change here 
	TString folder_direction = "/data4/chengwei/Geant4/INTT_simulation/G4/for_CW/Beamtest_data_Different_config/data_run64_with_lead_plate";
	...
	bool LoE_removal = true; // true : for the bad event that is because of the longitudinal axis offset will be neglected.
	...
	//title for setting ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	// double alignment_array [4]={0, -0.2908,0,0};  //offset matrix of 4 layers, Y axis; ** if the target sensor type is changed, the alignment needs to be re-studied 
	...
	for (int i=0; i<event_N; i++)
	{
		...
		// todo : the "INTT_event == 1" is removed from the line below. 2022/11/30
	...
	if (full_analysis == true)
	{
		...
		vector<vector<double>> cluster_adc_keeper; cluster_adc_keeper.clear();
		...
		double hit2_posA; 
		double hit2_posB;
		vector<double> hit2_posA_array; hit2_posA_array.clear();
		vector<double> hit2_posB_array; hit2_posB_array.clear();
		...
		TFile * cluster_info_output_file = new TFile(Form("%s/folder_%s/cluster_information_slope_adcinfo_NoCamac.root",folder_direction.Data(),output_folder_name.Data()), "RECREATE");
		...
		for (int i=0; i<event_event.size(); i++)
		{
			...
			for (int i10 = 0; i10 < 13; i10 ++) // note : chip, or you can call it slot.
			{
				...
				{
					all_cluster_info = data_all_edep_weight(data_all,i10+1,adc_convert);
