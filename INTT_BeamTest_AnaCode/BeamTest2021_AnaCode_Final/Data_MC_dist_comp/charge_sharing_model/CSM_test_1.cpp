...
void CSM_test_1 ()
{
    ...
    TString file_directory = "/data4/chengwei/Geant4/INTT_simulation/G4/for_CW/CSM_data/test_1_100event";
    TString file_name      = "run52_X0_Y-5_100Event_WithSciAlFoil_10PercentHDIMetalMaterial_BeamSmear_WVI_20umCut_Run52DataU8Offset_L1_2.91425e-01mm_RoughTrigger";

    //note : for the output
    TString output_file_name = "after_CSM";
    output_file_name = output_file_name + Form("_CW_%.3f",cloud_width) + Form("_nSubStep_%i",nsegments) + Form("_UsedSigma_%.3f",used_sigma) + Form("_DACOffset_%.0f",ADC_offset);
    ...
    for (int i = 0; i < N_step_tree; i++)
    {
        ...
        if (i % sample_rate == 0)
        {
            cout<<"loop index : "<<i<<" step info, eID : "<<eID<<" edep : "<<step_edep<<" \\ pre : "<<pre_X<<" "<<pre_Y<<" "<<pre_Z<<" "<<pre_process<<" \\ Post : "<<post_X<<" "<<post_Y<<" "<<post_Z<<" "<<post_process<<endl;
        }
    ...
    for (int i = 0; i <  event_array.size(); i++) // note : the for loop for the number of event
    {
        if(i % sample_rate == 0) cout<<"===================== ===================== ===================== runnting CC model : " << i << " ===================== ===================== ====================="<<endl;
        if(i % sample_rate == 0) cout<<"===================== ===================== ===================== runnting CC model : " << i << " ===================== ===================== ====================="<<endl; 
        if(i % sample_rate == 0) cout<<"===================== ===================== ===================== runnting CC model : " << i << " ===================== ===================== ====================="<<endl;
        for (int i1 = 0; i1 < event_array[i].size(); i1++) // note : the for loop for the step hit in the event
        {
            if(i % sample_rate == 0) cout<<"--------------------- event step, eID : "<<event_array[i][i1].eID<<" layer : "<<event_array[i][i1].layer_id<<" column : "<<event_array[i][i1].column_id<<" chan : "<<event_array[i][i1].chan_id<<" chan_center : "<<get_chan_id(event_array[i][i1].pre_Y,event_array[i][i1].post_Y).second<<" edep : "<<event_array[i][i1].edep<<endl;
            if(i % sample_rate == 0) cout<<"--------------------- total distance : "<<Get_XYZ_dist(event_array[i][i1])<<endl;
            if(i % sample_rate == 0) cout<<"--------------------- pre pos : "<<event_array[i][i1].pre_X<<" "<<event_array[i][i1].pre_Y<<" "<<event_array[i][i1].pre_Z<<"\\ post pos : "<<event_array[i][i1].post_X<<" "<<event_array[i][i1].post_Y<<" "<<event_array[i][i1].post_Z<<endl;
            if(i % sample_rate == 0) cout<<"--------------------- Dist, X : "<<fabs(event_array[i][i1].post_X - event_array[i][i1].pre_X)<<" Y : "<<fabs(event_array[i][i1].post_Y - event_array[i][i1].pre_Y)<<" Z : "<<fabs(event_array[i][i1].post_Z - event_array[i][i1].pre_Z)<<endl;
            if(i % sample_rate == 0) cout<<"~~~~~~~~~~~~~~~ n segments start ~~~~~~~~~~~~~~~"<<endl;
            ...
            {   
                ...
                for (int ct = 0; ct < sub_step_unit.involve_chan.first; ct++) // note : for loop for upper
                {
                    ...
                    if(i % sample_rate == 0) cout<<"index : "<<i2<<" upper channel add adc : "<<ct<<" edge low : "<<CW_edge_low<<" high : "<<CW_edge_high<<" edep portion : "<<get_portion_edep(sub_step_unit.CW, sub_step_unit.y, sub_step_unit.edep, CW_edge_low, CW_edge_high)<<endl;
                }
                for (int cb = 0; cb < sub_step_unit.involve_chan.second; cb++) // note : for loop for the bottom
                {
                    ...
                    if(i % sample_rate == 0) cout<<"index : "<<i2<<" bottom channel add adc : "<<cb<<" edge low : "<<CW_edge_low<<" high : "<<CW_edge_high<<" edep portion : "<<get_portion_edep(sub_step_unit.CW, sub_step_unit.y, sub_step_unit.edep, CW_edge_low, CW_edge_high)<<endl;
                }

                if(i % sample_rate == 0) cout<<"index : "<<i2<<" "<<sub_step_unit.x<<"\t"<<sub_step_unit.y<<"\t"<<sub_step_unit.z<<endl;
                if(i % sample_rate == 0) cout<<"cloud width : "<<sub_step_unit.CW<<" involved channel : "<<sub_step_unit.involve_chan.first<<" "<<sub_step_unit.involve_chan.second<<endl;
                if(i % sample_rate == 0) cout<<"chan_id : "<<sub_step_unit.chan_id<<" chan center : "<<sub_step_unit.chan_center<<endl;
            ...
            for (int k = 0; k < 13; k++)
            {
                for (int l = 0; l < 256; l++)
                {
                    if (chan_edep[j][k][l] > 0)
                    {
                        ...
                        if(i % sample_rate == 0) cout<<"test in chan_edep selection : "<<edep_convert<<" "<<chan_edep[j][k][l]<<endl;
