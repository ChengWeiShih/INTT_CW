...
void run52_U8_comp_scan(TString MC_folder_direction, TString MC_cluster_file_name)
{
    ...
    double data_cluster_adc_value_requirement = 22.5; // note : it's a cut
    ...
    // TString MC_folder_direction = "/data4/chengwei/Geant4/INTT_simulation/G4/for_CW/BeamTest2021_data_MC_file_plot_final/MC_file/MC_run52/folder_run52_MC_run_20230629_4/new_class_attempt";
    // TString MC_cluster_file_name = "cluster_information_offset-0.0000_adcinfo_NoCamac";
    int MC_study_chip = 11;
    double MC_cluster_adc_value_requirement = 22.5; // note : it's a cut
    ...
    string MC_folder_direction_string = MC_folder_direction.Data(); 
    string MC_config = MC_folder_direction_string.substr(MC_folder_direction_string.find("folder_e+_934MeV"),MC_folder_direction_string.length());
    pair<double,double> ratio_Y_range_pair = {-3. + 1, 3 + 1};
    
    vector<TString> titles_scattering_vec = {"Scattering [slope_{l2l1} - slope_{l1l0}]","A.U."};
    TString plot_scattering_name = Form("DataRun52U8_MC%s_scattering_%s_",file_txt.Data(),MC_config.c_str());
    ...
    TString plot_residual_name = Form("DataRun52U8_MC%s_residual_%s_",file_txt.Data(),MC_config.c_str());
    ...
    // note : l1_alignment hist
    vector<TString> titles_l1alignment_vec = {"l1 alignment [mm]","A.U."};
    TString plot_l1alignment_name = Form("DataRun52U8_MC%s_l1alignment_%s_",file_txt.Data(),MC_config.c_str());
    dataMC_comp (data_run52_U8 -> Get_l1_alignment_hist(), MC_run52_U8 -> Get_l1_alignment_hist(), MC_folder_direction , plot_l1alignment_name, titles_l1alignment_vec, ratio_Y_range_pair,false, false);

    // note : l0l2 slope hist
    vector<TString> titles_l0l2slope_vec = {"slope","A.U."};
    TString plot_l0l2slope_name = Form("DataRun52U8_MC%s_l0l2slope_%s_",file_txt.Data(),MC_config.c_str());
    dataMC_comp (data_run52_U8->Get_l0l2_slope_hist(), MC_run52_U8 -> Get_l0l2_slope_hist(), MC_folder_direction , plot_l0l2slope_name, titles_l0l2slope_vec, ratio_Y_range_pair, false, false);

    // note : l0l1 slope hist
    vector<TString> titles_l0l1slope_vec = {"slope","A.U."};
    TString plot_l0l1slope_name = Form("DataRun52U8_MC%s_l0l1slope_%s_",file_txt.Data(),MC_config.c_str());
    dataMC_comp (data_run52_U8 -> Get_l0l1_slope_hist(), MC_run52_U8 -> Get_l0l1_slope_hist(), MC_folder_direction , plot_l0l1slope_name, titles_l0l1slope_vec, ratio_Y_range_pair ,false, false);
