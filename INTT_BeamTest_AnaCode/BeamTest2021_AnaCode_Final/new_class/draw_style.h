...
void Characterize_Rate1F (TH1F *hist,  int hcolour, pair<double,double> Y_range_pair={0.4,3.2})
{
	float ratio = 7.0/3.0;
	hist -> SetTitle       ("");
...
// note : titles_vec[0] : x
// note : titles_vec[1] : y
void dataMC_comp (TH1F* hist_data, TH1F* hist_MC, TString folder_direction, TString plot_name, vector<TString> titles_vec, pair<double, double> ratio_Y_range_pair = {0.4, 3.2} ,bool linear_or_log = false, bool statsbox_bool = false)
{
    ...
    Characterize_Rate1F(hist_ratio,1, ratio_Y_range_pair);
