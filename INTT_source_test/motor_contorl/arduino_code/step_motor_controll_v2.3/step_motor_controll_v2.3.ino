  ...
  if(checkpoint == true){
    ...
    if (strcmp(mode_message, "single") == 0){
      ...
      while(true){
          ...
          if(ID <= 26 && ID >= 1){
            ...
            if((ID <= 5 && ID_pre > 5) || (ID_pre <= 5 && ID > 5)){
              ...
              if(ID <= 5){
                Serial.println("ID <= 5");
                delay(100);
                motor_steps(-(steps_gap_B*(5 - ID)+ AB_gap + steps_gap_A*(ID_pre - 6)));
                delay(100);
              }
              else{
                Serial.println("else");
        ...
        if(ID_next == true){
          Serial.println("Entering next ID or done:");
    ...
    if (strcmp(mode_message,"free") == 0){
      bool done = false;
      bool step_read = false;
      bool step_move = false;      
      bool step_done = false;
      bool step_trans = false;
      ...
      char step_message[32];
      int step_message_pos = 0;
      ...
      while(true){
        ...
        while(Serial.available() > 0 && step_read == false){
          char command = Serial.read();
          if (command != '\n'){     
            step_message[step_message_pos] = command;
            step_message_pos++;
          }
          else{
            Serial.read();
            step_message[step_message_pos] = '\0';
            step_message_pos = 0;
            step_read = true;
          }
        }
        ...
        if(step_read == true){
          if(isAlpha(step_message[0]) || isAlpha(step_message[1])){
            step_done = true;
            break;
          }
          else{
            steps = atoi(step_message);
        ...
        if(step_move == true){
          Serial.println("Entering steps or done:");
